...
image_array_train_min = np.amin(X_train_org)
image_array_train_max = np.amax(X_train_org)
image_array_validation_min = np.amin(X_test_org)
image_array_validation_max = np.amax(X_test_org)

tm_sample_10 = TMClassifier(
    number_of_clauses=2000*factor,
    T=500*factor,
    s=10.0,
    max_included_literals=max_included_literals,
...
for epoch in range(100):
    ...
    random_train_matrix = np.random.randint(image_array_train_min,image_array_train_max, size=(X_train_org.shape))
    random_validation_matrix = np.random.randint(image_array_validation_min,image_array_validation_max, size=(X_test_org.shape))

    # Sampeled images
    floaty_train_images = np.greater(random_train_matrix,X_train_org)
    floaty_validation_images = np.greater(random_validation_matrix,X_test_org)

    tm_sample_10.fit(X_train_thermometer, Y_train)
    Y_test_sample_10, Y_test_scores_sample_10 = tm_sample_10.predict(X_test_thermometer, return_class_sums=True)
    print("4x4 Color Thermometers: %.1f%%" % (100*(Y_test_sample_10 == Y_test).mean()), end=' ')
    ...
    for i in range(Y_test.shape[0]):
        ...
        votes[i] += 1.0*Y_test_scores_hog[i]/(np.max(Y_test_scores_sample_10) - np.min(Y_test_scores_sample_10))
